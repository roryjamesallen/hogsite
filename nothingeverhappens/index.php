...
function unixToUsefulText($unix){
    ...
    $hours = floor(unixToHours($unix) - ($days * 24) - ($weeks * 168));
    $minutes = floor(unixToMinutes($unix) - ($hours * 60)  - ($days * 1440) - ($weeks * 10080));
...
function sendNotificationEmails($event_id, $email_setting){
    ...
    if ($email_setting == 'new_event'){
        ...
        $message = 'Event resolved/cancelled by '.getUsernameById($user_id).' in '.getGroupNameById($group_id).': '.getEventQuestionById($event_id).'. View the event to see the full results.';
    }
