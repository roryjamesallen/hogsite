...
if ($combo_id != null){ // Combo already exists in combos table
    ...
    $combo_id = uniqid();
    sqlQuery('INSERT INTO combos (combo_id, main, snack, drink, ip, time) VALUES ("'.$combo_id.'", "'.urlencode($main).'", "'.urlencode($snack).'", "'.urlencode($drink).'", "'.$ip_address.'", "'.time().'")'); // Insert the combo
}
...
if ($combo_name == null){ // As long as it doesn't already have a name
    sqlQuery('INSERT INTO names (name_id, combo_id, name, username, ip, time) VALUES ("'.uniqid().'", "'.$combo_id.'", "'.urlencode($name).'", "'.urlencode($username).'", "'.$ip_address.'", "'.time().'")'); // Insert the name
